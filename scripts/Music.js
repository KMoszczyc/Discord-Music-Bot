...
class Music {
    ...
    resume(message, serverQueue) {
        if (this.isQueueEmpty(serverQueue))
            return Utils.shortEmbedReply(message, `There is nothing to resume!`);
        else {
            console.log('resume');
    ...
    pause(message, serverQueue) {
        if (this.isQueueEmpty(serverQueue))
            return Utils.shortEmbedReply(message, `There is nothing to pause!`);
        else {
            console.log('pause');
    ...
    async findLyrics(message, serverQueue) {
        if (serverQueue && serverQueue.songs !== [] && serverQueue.connection) {
            ...
            Utils.shortEmbedReply(message, 'The queue os empty!');
    }
    ...
    async createQueue(message, voiceChannel) {
       ...
       await this.connectBot(message.guild.id, voiceChannel, this.queue.get(message.guild.id)).then(conn => {
            if (conn) 
                this.play(message.guild, this.queue.get(message.guild.id).songs[0]);
        });
    }
    ...
    async skipCommand(message, serverQueue) {
        if (this.isQueueEmpty(serverQueue))
            return Utils.shortEmbedReply(message, `There is nothing to skip!`);
        serverQueue.connection.dispatcher.end();
    }
    ...
    async clearQueueCommand(message, serverQueue) {
        if (this.isQueueEmpty(serverQueue))
            return Utils.shortEmbedReply(message, `There is nothing to clear!`);
    ...
    async play(guild, song) {
        ...
        const reply = new Discord.MessageEmbed()
            .setDescription(`Now we play: [${song.full_title}](${song.url})! \t`)
            .setColor(0xa62019);
